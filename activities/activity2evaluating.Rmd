...
A 1 permutation and 5 folds cross-validation using default selection and classification methods is done using `runTests`. Read its [documentation](https://bioconductor.org/packages/release/bioc/html/ClassifyR.html) to understand the options available.
...
As was done for DM classification, 1 permutation and 5 fold cross-validation is done. It takes substantially longer to complete than DM classification, because thousands of kernel densities are being esimated for each iteration.
